...
function loadIPFSDeployment(): IPFSDeployment {
  if (!fs.existsSync(IPFS_DEPLOYMENT_FILE)) {
    console.error(`❌ Error: IPFS deployment file not found at ${IPFS_DEPLOYMENT_FILE}`);
    console.log("Please run 'pnpm publish-ipfs' first to generate the IPFS deployment file");
    process.exit(1);
  }
...
async function createWeb3Gateway(ipfsHash: string): Promise<CloudflareWeb3Gateway> {
...
async function findExistingGateway(): Promise<CloudflareWeb3Gateway | null> {
  ...
  try {
    ...
    if (response.ok && data.success && data.result) {
      const existingGateway = data.result.find(gateway => gateway.name === GATEWAY_HOSTNAME);
...
async function updateWeb3Gateway(gatewayId: string, ipfsHash: string): Promise<CloudflareWeb3Gateway> {
  const dnslink = `/ipfs/${ipfsHash}`;
...
async function main(): Promise<void> {
  ...
  if (showHelp) {
    ...
    console.log("\nOptions:");
    console.log("  --create, -c    Force creation of a new gateway (fails if gateway exists)");
    console.log("  --update, -u    Force update of existing gateway (fails if gateway doesn't exist)");
  ...
  if (forceCreate && forceUpdate) {
    ...
    console.log("Usage: tsx script/set-dns-record.ts [--create|-c] [--update|-u] [--help|-h]");
    process.exit(1);
  }
  ...
  if (existingGateway) {
    ...
    if (forceCreate) {
      console.error("❌ Error: Gateway already exists but --create flag was specified");
    ...
    if (currentHash === deployment.ipfsHash && !forceUpdate) {
      console.log("✅ Gateway is already pointing to the current IPFS hash - no update needed!");
    ...
    if (forceUpdate) {
      console.error("❌ Error: No existing gateway found but --update flag was specified");
  ...
  console.log("   The gateway will automatically create the necessary DNS records.");
}
